# ============================================
# STACK CONFIGURATION: Stage Compute
# ============================================
# This file defines the **stack-level orchestration** of all Terraform modules
# required to deploy a compute staging infrastructure on OpenStack.
# It integrates compute, storage, and load balancing components.
#
# Modules Included:
#
...
module "loadbalancer" {
  ...
  external_network_name = var.external_network_name
  network_id            = data.terraform_remote_state.network.outputs.network_id
  subnet_id             = data.terraform_remote_state.network.outputs.subnet_ids["web"]
